...
- The scanner (initial scan and rescans), to populate the folder's
  `library.db`.
...
edits stay in the DB via `library::apply_metadata_patch`.
...
merged `ImageMetaFromFile` is then written to the folder's
`library.db` via `library::set_image_meta`.
