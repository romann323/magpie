...
- Persists user tags in a single, private SQLite database in the
  user's Windows AppData folder (`%APPDATA%\com.magpie.app\magpie.db`).
  Source folders on disk stay untouched.
...
| Tag storage             | Single SQLite database at `%APPDATA%\com.magpie.app\magpie.db`. Every recognised file type is fully taggable. |
...
| Storage                 | Single SQLite database (`magpie.db`) under `%APPDATA%\com.magpie.app\` + WebP thumbnail cache in the same folder. |
...
2. **Never move or rename files.** The user is in charge of their
   folder hierarchy.
3. **Tags stay on the user's PC.** They live in `magpie.db` under
   `%APPDATA%`; nothing is ever written into the source folders.
...
5. **Fast enough to be daily-drivable.** All hot paths are on
   native Rust; the UI is virtualised; a single SQLite database
   comfortably handles hundreds of thousands of images with FTS5.
